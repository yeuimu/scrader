...
// 用法：node vision_loop.js --title Edge --goal "把 Issues 排序恢复为默认排序" [--type-text 文本] [--max-steps 6] [--port 8765] [--max-dim 768]
'use strict';
const fs = require('fs');
  ...
  try {
    for (let step = 1; step <= maxSteps; step++) {
      ...
      const d = await mcpDecide({
        state: { task: goal, history, screenshot_size: [g.image.width, g.image.height], candidates: cands.map((e) => ({ id: e.id, text: (e.text || '').slice(0, 24), center_px: e.center_px })) },
        questions: { next: { type: 'choice', criteria, instructions: `当前任务：${goal}。历史动作 ${history.length ? JSON.stringify(history) : '（无，第一步）'}——若某动作已做过且页面无变化，勿再重复，换路线。点击后新出现的弹层/菜单项也会出现在候选里。目标已完成选 goal_done，无法推进选 stuck。` } },
      });
      ...
      if (ans.choice === 'goal_done') { console.log('✅ goal_done'); break; }
      if (ans.choice === 'stuck') { console.log('⛔ stuck'); break; }
