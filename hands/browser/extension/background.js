...
importScripts('vendor/trajectory.js'); // 拟人轨迹算法（构建产物，源头 motion/trajectory.js）
...
// 拟人轨迹：消费 motion 的 timeline（算法唯一源；本 CDP 后端与 hands/cua/glide.js 的 SendInput 后端吃同一张表）
async function humanMove(t, x0, y0, x1, y1) {
  const tl = buildTimeline(x0, y0, x1, y1);
  const t0 = Date.now();
  for (const s of tl.steps) {
    await chrome.debugger.sendCommand(t, 'Input.dispatchMouseEvent', { type: 'mouseMoved', x: s.x, y: s.y, button: 'none' });
