...
const { encodeFrame, decodeFrames } = require('./bridge.js');
...
// ── MCP 客户端（stdio） ──
function mcpSession() {
  const child = spawn(process.execPath, [path.join(__dirname, 'scrader-mcp.js')], {
    env: {
      ...process.env,
...
(async () => {
  ...
  const bridge = spawn(process.execPath, [path.join(__dirname, 'bridge.js')], { env: { ...process.env, SCRADER_PORT: String(PORT) }, stdio: ['ignore', 'pipe', 'pipe'] });
  ...
  const mcpMod = require('./scrader-mcp.js');
